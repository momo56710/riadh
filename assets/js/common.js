	...
	$(function () {
		...
		*/
		$(document).ready(function () {
			var defaultFilterValue = '.sorting-ui-ux-design';

			// Add 'active' class to default filter link
			$('.filter-links a[data-href="' + defaultFilterValue + '"]').addClass('active');

			// Show the default filter value on page load
			$container.isotope({ filter: defaultFilterValue });

			// Add animate__active class to the default filter section if not already present
			if (!$(defaultFilterValue).find('.scroll-animate').hasClass('animate__active')) {
				$(defaultFilterValue).find('.scroll-animate').addClass('animate__active');
			}

			// Isotope filter click handler
			$('.filter-links').on('click', 'a', function () {
				...
				// Add animate__active class to the clicked filter section if not already present
